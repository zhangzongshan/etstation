...
namespace PublicClass.Common
{
    class Common
    {
        ...
        {
            string strPathTemp = path;
            try
            {
                string[] dirTemp = strPathTemp.Split("\\".ToCharArray());
                string dirPathTemp = "";
                for (int i = 0; i < dirTemp.Length - 1; i++)
                {
                    dirPathTemp = dirPathTemp + dirTemp[i] + "\\";
                    if (dirPathTemp != "")
                    {
    ...
    public class ApiException : Exception
    {
        ...
        public ApiException(string message, string errorCode): base(message)
        {
            ...
            result.Add("resultObject", null);
            result.Add("status", "fail");
            result.Add("code", errorCode);
            result.Add("message", message);

            HttpResponseMessage res = new HttpResponseMessage { Content = new StringContent(JsonConvert.SerializeObject(result), Encoding.GetEncoding("UTF-8"), "application/json") };
